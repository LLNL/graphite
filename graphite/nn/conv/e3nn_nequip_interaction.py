...
class E3NN_Interaction(torch.nn.Module):
    ...
    * https://github.com/e3nn/e3nn/tree/0.4.4/e3nn/nn/models/v2106.
    ...
    Args:
        ...
        irreps_node (e3nn.o3.Irreps or str): Irreps of node attributes (constant throughout model).
        irreps_edge (e3nn.o3.Irreps or str): Irreps of edge spherical harmonics (constant througout model).
    ...
    def __init__(self,
        ...
        assert irreps_mid.dim > 0, f"irreps_in={self.irreps_in} times irreps_edge={self.irreps_edge} produces nothing in irreps_out={self.irreps_out}."
        ...
        with torch.no_grad():
            self.alpha.weight.fill_(0.)
        assert self.alpha.output_mask[0] == 1.0, f"irreps_mid={irreps_mid} and irreps_node={self.irreps_node} are not able to generate scalars."

    def forward(self, x, node_attr, edge_index, edge_attr, edge_len_embbed):
        i, j = edge_index
        num_nodes = x.shape[0]
        ...
        edge_features = self.conv(node_features[i], edge_attr, weight=self.mlp(edge_len_embbed))
