...
class DPMSolverDiffuser:
    """Specialized DPM implementation based on the paper titled
    'DPM-Solver: A Fast ODE Solver for Diffusion Probabilistic Model Sampling in Around 10 Steps'.

    Reference: https://arxiv.org/pdf/2206.00927.pdf
    """
    def __init__(self, schedule='linear', t_min=1e-3):
        """Instantiate the noise schedules of DPM_solver.
        """
    ...
    def forward_noise(self, x, t):
        """Applies forward noising to 'x' at 't'.
        Returns the noised version 'x_t' and the sampled standard Gaussian noise `eps`.
        """
        eps = torch.randn_like(x)
        x_t = self.alpha_t(t)*x + self.sigma_t(t)*eps
        return x_t, eps

    def reverse_denoise(self, x_T, noise_net, solver, M=20):
        """Apply reverse denoising to 'x_T' (typically a random Gaussian noise)
        with a learned noise prediction model.
        """
