...
def cell_shift(coord, shape):
    return torch.div(coord, shape, rounding_mode='floor')
...
def periodic_radius_graph(pos, box, cutoff):
    ...
    # Construct the `cells` data structure, where each row is a cell, and the columns
    ...
    nbr_disp       = torch.cartesian_prod(*(torch.arange(-1, 2) for _ in range(num_dims))).to(pos.device)
