...
class NequIP_dpm(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.embed_node_x   = nn.Embedding(num_species, self.irreps_in.dim)
    ...
    def forward(self, x, t, sigma=1.0):
        """Predict noise/score given a noised input `x` and schedule time `t`.

        Args:
            x: The atomic structure to be de-noised.
